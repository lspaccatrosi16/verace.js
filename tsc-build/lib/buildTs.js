...
export default function (config, log) {
    return new Promise((resolve, reject) => {
        if (existsSync("src/index.ts")) {
            try {
                ...
                if (config.test != "") {
                    ...
                    if (testRes.error) {
                        ...
                        if (testRes.status != 0)
                            throw testRes.status;
                    }
                }
                execSync(`npx esbuild tsc-build/index.js --outfile="build/veraceTemp.cjs" --bundle --platform=node --target=node16`, { stdio: "inherit" });
                writeFileSync("build/index.cjs", veraceTempJS);
                execSync(`npx esbuild build/index.cjs --outfile="dist/${config.name}.cjs" --bundle --platform=node --target=node16`, { stdio: "inherit" });
            ...
            reject("src/index.ts was not found");
        return;
    });
}
